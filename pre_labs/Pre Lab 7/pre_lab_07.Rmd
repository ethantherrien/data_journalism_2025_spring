...
```{r echo=FALSE}
#census_api_key("0bbd6c1e04dfa5487d9a9d49c8daaa8eeeec3f3d", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** Nevada. Percentage change of about 35.1%.
...
Is the difference between Calvert and Montgomery significant? **Answer** Not really, but sort of. There is a notable gap, but Howard is number 1 on the list and Calvert is number 2. So rankings wise, the differnecs is not significant.

Is the difference between Howard and everyone else significant? **Answer** Yes. Howard laps the field in terms of household income. It is number one by a decent bit, and is far, far ahead of most counties.
...
**Answer** Unfortunately, this block of code is not running for me, even after fiddling with it for about ten minutes. However, I can assume that zip code 21740 is one of them, but I can't tell for sure as this code refuses to load properly for me.
...
**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? **Answer** What kinds of areas are these places? What specific neighborhoods are contributing to this high median household income? What are the most common occupations for individuals who live in this zip code?

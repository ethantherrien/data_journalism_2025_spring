...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** A dataset with a bunch of numbers and terms from different elementary schools. Columns include precinct names, polling IDs and shapes. I don't understand what all of this means, but... 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** Precinct information from a bunch of different schools. It also has this column called "shape," but im not 100% sure what that means.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A map of Prince Georges' County with coordinate data. It shows precincts too.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Warning: package ‘tigris’ was built under R version 4.4.3To enable caching of data, set `options(tigris_use_cache = TRUE)`
...
**Task** Run the following code. Describe the output in the space below. **Answer** A very confusing dataset with a bunch of county names and numbers. Three columns have red NAs. This looks like state and county data.
...
**Task** Run the following code. Describe the output in the space below. **Answer** County data. These terms/columns in the dataset make zero sense, but I know it is county-based data.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A map of Maryland with coordinate data, showing county borders.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Another map of Prince Georges' County with precinct borders and coordinate data. I can't tell if this is different from the first one, but it appears to be slightly so.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Polling numbers, differentiating by early voting, absentee, provisional and total eligible.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It shows precincts by school name, and then shows voter turnout.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A blue map of Prince Georges' County. It would appear this creates a map that measures turnout by precinct by shade of color.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A dataset by county that shows voter numbers by several political parties.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It shows Maryland counties and then their voting population.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Data from the 5-year ACS that shows Maryland counties along with a "shape" column of sorts.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Maryland voting data by county, along with individual party data numbers. It also shows a total at the end.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The same county dataset with the "shape" or geometry portion. It also includes a population column.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This joins the datasets. It now shows counties, population, party data and shape/geometry.
...
**Task** Run the following code. Describe the output in the space below. **Answer** County name, population and geometry all in one dataset.
...
**Task** Run the following code. Describe the output in the space below. **Answer** County names, population, party data, shape/geometry and now a voters per 10k section.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A map of Maryland showing coordinate data and county borders. But now, it shows voters per 10k in each county by indicating through a color shade scale, making this visualized data.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It changed the colors, making this a bit easier to understand. It now has shades of red and purple.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** A similar clumb in the counties around DC, around Baltimore nd in northern Maryland. They each have their own habits.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The same Maryland voters per 10k dataset, but it adds a una voters per 10k column now. I assume this is unaffiliated voters.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer** There arent a lot of unaffiliated voters in or around major cities (Baltimore and the area around DC). There are a lot in counties like Montgomery. Its in between in Marylands less populated counties east of the Chesapeake.

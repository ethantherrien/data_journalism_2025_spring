...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **Data type for last_name is chr. Data type for amount is dbl. I would think that last name identifies the person making the contribution, and amount signifies how much money the contribution was.** 
...
**The results from head() were providing the first and last name of the people who contributed. These results here provide the number of total contributions made every single day from April 1 to June 30, in order by calendar day.**
...
**The data now ranks the days of contributions in order from the day with the most contributions to the last. It is an effective way of providing data showing the most productive days in the contribution process.**
...
**The new codeblock now asks to group by memo_text and amount rather than by date. So now, we get a list of where those contributions were made rather than when they were made.**
...
**The codeblock now asks to list the data in order from total_amount. So now, we get the same data of where the contributions were made but instead, it's now in order from highest contribution to lowest.**
...
**The codeblock now requests median and mean amounts, which provides it in the ensuing data table. The biggest top result from Maryland is the HOGAN FOR MARYLAND fund **
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **The codeblock now asks for a min_amount (smallest contribution made) and a max_amount (largest contribution made). The data then provides this information. From what I can see, it looks like the data is sorted so that the city with the most total contributions is listed first, and then it goes from there in descending order.**

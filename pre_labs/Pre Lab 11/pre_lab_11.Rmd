...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** They have long titles, which is probably a problem. But the columns themselves are super clean. There isn't a lot of work to do in the cleaning department. Numbers are properly formatted with correct comma placement.
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **Answer** They're referring to the corresponding column (2nd column, 3rd column) and then renaming them based on what the rename function commands it to do.
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer** There aren't very many of them.
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer** A long list of outputs with a bunch of /n tags. Not sure what those mean. This could probably be cleaned up.

...
**Task** Run the following code and describe the dataframe it outputs. **Answer** There are four columns: date, title, url and text. This appears to be a press release dataset from Senator Ben Cardin, showing all of his press releases in reverse chronological order. The text column shows the byline and a short description of the release.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It filters out unique words. For example, "dog" is said twice, but after running the code, it is only once. It combines multiple instances of words into one instance, if that makes sense.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It looks like this extracts every unique word from the press releases and puts it into a database? The output of this code is massive. It appears to have an instance of every single word that was used in these press releases.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** Interesting? Definitely not. I frankly don't give a damn how many times the word "the" appears in Ben Cardin's press releases.

To describe the graphic, that is pretty much what it is. It siphons out the most common words used in Cardin's press releases and makes a horizontal bar chart to depict the most common uses.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** It is a database showing unique words, how many times they were used and then a "percent" of sorts. I think that percent is what percentage of the press releases is taken up by that specific word. I think whether or not this is more interesting than before is totally subjective, but one could argue that it is simply because of the added percentage. I still think it's boring.
...
**Task** Run the following code and describe the results. **Answer** This gives me the unique words from 2024 and 2022 press releases, I think? It makes two separate databases. It also provides that same percentage. This appears to be the same as the last code/database, it just siphons the data into two separate years.
...
**Task** Run the following code and describe the results. **Answer** This filters by 2022. This appears to filter by joint words or two word phrases, giving a count of how many times they occurred, and then providing the aforementioned percentage.
...
**Task** Run the following code and describe the results. **Answer** This does the same thing as the last code block, but for the year 2024 instead. It siphons out two word phrases, gives a count and a percentage.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** One siphons out the words, the other makes them all present test. The second one turns "drove" and "driven" into "drive." I think the second one is more useful, as it allows us to better encapsulate how many times a phrase is used.
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** This one is siphoning the top 10 words and creating two databases, one that lemmatizes the words and the other that doesn't. For example, the first one turns "funding" into "fund." The second one does not. I think the second one is more useful, as it allows us to better encapsulate how many times a phrase is used.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** It creates two new databases, one for 2022 and one for 2024. The data siphons out those words and provides a count, but it also lists the sentiment of that word, whether it was positive or negative. Some of them do seem incorrect or counter-intuitive. Work being positive? Depends on who you ask.

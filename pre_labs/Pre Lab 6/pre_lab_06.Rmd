...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** For one, the rows have a ton of NAs in them. Also, the column names make zero sense. With names like "391" and "716," how are people supposed to read this data? It isn't clean.
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** The NAs are now gone, making it cleaner... but the column names still make zero sense. X1 and X2 are not proper candidate names. This still isn't clean.
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** The column names are actual candidate names now, making this data cleaner. That much is fine. However, there are still a number of issues. On the second page, there are a ton of NAs in the "office" column. That isn't clean. Also, I feel as if these columns could potentially be clustered so that each precinct only has one. It seems a bit discombobulated between the excessive rows/columns and the number of NAs I see. However, as this document states below, this may be as best as we can get it without manually editing the data ourselves. But this data certainly isn't perfect or clean, by any means.

...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer** There are 1640 rows, so I would assume there are that many problems. I'm not really sure what the problem is but I would assume these are all of the Texas precincts, and there were issues retrieving their data.
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** I don't know for sure? I think the bottom two messages about specifications are both parsing errors. But nothing immediately stands out to me as a warning.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** District is <dbl>, Precinct is <chr> and Mail is also <dbl>
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: 402345. This value represents the number of votes that WEREN'T mail-in (hence the NA and hence the high number).
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** Unavailable essential information is definitely worth investigation. Why is it unavailable? There could be something there. Also, why are there so many in the "deceased" section? Could also be worth investigating.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** <chr>
...
**Answer** Inconsistent capitalization across columns, missing values in a lot of columns and the zip codes column mixes a bunch of five digit and nine digit zip codes.
...
**Answer** it adds an "x" at the start of it to say x1_linenumber
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** Error in `summarise()`:
ℹ In argument: `total_amount = sum(amount)`.
Caused by error in `sum()`:
! invalid 'type' (character) of argument

I think the issue is that sum(amount) doesn't mean anything because the function isn't giving R anything to add. The sum of what?
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** Every zip code now has five digits, and they're all the same zip code.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** One mispelling remains.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** Zero mispellings remain.

...
**Answer** The code is creating a database of every candidate involved in the election, and then calculating what percentage of their votes came on election day. That's what the percentage election day command means.
...
**Answer** The results are providing a percentage for election day vote count rather than a decimal. Also, it's now ordered from highest election day percentage to lowest. This is because of the arrange(desc) command added to the code, which arranges the results in descending order from highest to lowest.
...
**Answer** I would say that yes, it is a more interesting and useful answer. I feel like in general, the majority of people vote on election day. So why would figuring out which candidates got the highest percentage of their votes on election day be interesting? Getting a high percentage is normal and expected. What ISNT expected is getting a low percentage. When ordered from lowest to highest, the question is raised: why are these candidates getting such a low percentage of their votes on election day? This is a legitimate story and something to look into. The one before this is not.
...
**Answer** One is str_to_lower. This function converts text to lowercase letters. A second function is str_to_title. This function capitalizes the first letter of each word in a sentence or text block.
...
**Answer** This mutate statement is essentially acting like a filter. It filters the dataset into different conditions and blocks based on the data it requests. The first is asking for all contributions that were less than $100 and then filtering it into the "less than $100" amount_category. The next asks for between $500 and $1499 and filters it into a corresponding amount_category. This continues for every category requested.
...
**Answer** Less than $100. This category has the most money from contributions pumped into it.
...
**Answer** It keeps the information more condensed into one screen, rather than needing multiple tabs to filter through the information.
...
**Answer:** It no longer has the <chr> column name, so R no longer reads it as a character column. It now has the <date> column name, meaning R is reading it as a date column and the outputs as actual dates rather than just a set of numbers.
...
**Answer** The data is showing the total amount spent by month and then ordering the amount totals in descending order from highest to lowest. The new "month" columns aren't actually factoring in the exact day --- only the month and year are being considered. The total values spent in that month are then ordered from highest to lowest.
